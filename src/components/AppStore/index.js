...
class AppStore extends Component {
  state = {activeTab: tabsList[0].tabId, searchData: ''}

  onSearch = event => {
    const searchedValue = event.target.value
    this.setState({searchData: searchedValue})
  }
        ...
        <ul className="ul-style">
          {tabsList.map(eachList => (
            <TabItem
              details={eachList}
              key={eachList.tabId}
              active={eachList.tabId === activeTab}
              changeTab={this.onChangeTab}
            />
          ))}
        </ul>

        <ul className="apps-display-container">
          {searchedList.length > 0 ? (
            searchedList.map(eachApp => (
              <AppItem details={eachApp} key={eachApp.appId} />
            ))
          ) : (
            <p className="app-name">No App Found</p>
          )}
        </ul>
      </div>
    )
